    ...
    switch (opcode & 0xF000) {
      ...
      case 0x8000:
        switch (opcode & 0x000F) {
          // case 0x1,0x2,0x3,0x4 aadarsha
        case 0x4:                      // 8xy4 — Math Add with Carry Check
            uint16_t sum = V[regX] + V[regY];
            V[0xF] = (sum > 255) ? 1 : 0;
            V[regX]   = sum & 0xFF;
            break;

        case 0x5:                       // 8xy5 — Math Subtract
            uint8_t carry = (V[regX] >= V[regY]) ? 1 : 0;
            V[regX]   = V[regX] - V[regY];
            V[0xF] = carry;
            break;
        
        case 0x6:                       // 8xy6 — Bit Shift Right
            uint8_t dropped_bit = V[regX] & 0x1;
            V[regX] >>= 1;
            V[0xF] = dropped_bit;
            break;
        
        case 0x7:                       // 8xy7 — Alt Subtract
            uint8_t carry = (V[regY] >= V[regX]) ? 1 : 0;
            V[regX]   = V[regY] - V[regX];
            V[0xF] = carry;
            break;
        
        case 0xE:                       // 8xyE — Bit Shift Left
            uint8_t dropped_bit = (V[regX] >> 7) & 0x1;
            V[regX] <<= 1;
            V[0xF] = dropped_bit;
            break;
        }
